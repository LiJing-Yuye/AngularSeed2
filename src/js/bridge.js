...
function underline2Camel(obj) {
    if (typeof obj === 'string') {
        return obj.replace(/_([a-z])/g, function (g) { return g[1].toUpperCase(); });
    }
    var res = {};
    for (var propertyeName in obj) {
        var newPropertyeName = propertyeName.toLowerCase().replace(/_([a-z])/g, function (g) { return g[1].toUpperCase(); });
...
Native.prototype.gdprAcceptShareCrashAndUsage = function (str) {
    ...
    if (this.platform == "MAC") {
        res = window.webkit.messageHandlers.gdprAcceptShareCrashAndUsage.postMessage(param);      
    }
    else {
...
Native.prototype.userEditDeviceInfor = function (str) {
    ...
    if (this.platform == PLATFORM_TYPE.MAC) {
        res = window.webkit.messageHandlers.userEditDeviceInfor.postMessage(param);      
    }
    else {
        res = "";
        setTimeout(function() {
...
Native.prototype.getDeviceHistory = function (str) {
    ...
    if (this.platform == PLATFORM_TYPE.MAC) {
        ...
        res = window.external.OnMsgFromWeb(NativeCMD.CMD_RESULT_GET_HIS_DEVICES, param);
        setTimeout(function() {
...
Native.prototype.getISCHistory = function (str) {
    ...
    if (this.platform == PLATFORM_TYPE.MAC) {
        ...
        res = window.external.OnMsgFromWeb(NativeCMD.CMD_RESULT_GET_HIS_ISCVULS, param);
        //put it to asyn
        setTimeout(function() {
            setISCHistory(res);
        }, 0);
...
Native.prototype.getAirSupportState = function () {
    ...
    if (this.platform == PLATFORM_TYPE.MAC) {
        ...
        result = window.external.getAirSupportState();
...
if (location.href.indexOf("?debug") !== -1) {
    ...
    document.write('<script src="Common/js/debug.js" type="text/javascript"></script>');
    $(function() {
        //NewNetworkPage and NewDevicePage 's border in CSS
        if (window.location.href.indexOf("NewNetworkPage") == -1 && window.location.href.indexOf("NewDevicePage") == -1) {
...
function updateDeviceInfo() { broadcast('updateDeviceInfo', arguments); }
function updateSDKStatus() { broadcast('updateSDKStatus', arguments); }
function updateISCInfo() { broadcast('updateISCInfo', arguments); }
function currentWifiChanged() { broadcast('currentWifiChanged', arguments); }
function ScanErrorHandle() { broadcast('ScanErrorHandle', arguments); }
function goToUIPage() { broadcast('goToUIPage', arguments); }
function updateUIState() { broadcast('updateUIState', arguments); }

function setAirSupportState() { broadcast('setAirSupportState', arguments); }
function setAllSSID() { broadcast('setAllSSID', arguments); }
function setDeviceHistory() { broadcast('setDeviceHistory', arguments); }
function setISCHistory() { broadcast('setISCHistory', arguments); }
function setDeviceOpenConsole() { broadcast('setDeviceOpenConsole', arguments); }
function setHostMac() { broadcast('setHostMac', arguments); }
function updateDevOLStatus() { broadcast('updateDevOLStatus', arguments); }
function updateDeviceOLStatus() { broadcast('updateDevOLStatus', arguments); }
function notifyUserWithPopup() { broadcast('notifyUserWithPopup', arguments); }

function inform_isc_one_device_finish() { broadcast('informIscOneDeviceFinish', arguments); }
function installFinish() { broadcast('installFinish', arguments); }
function uninstallFinish() { broadcast('uninstallFinish', arguments); }
function update_one_new_device() { broadcast('addNewDevice', arguments); }
function updateDimaondBoxExist() { broadcast('updateDimaondBoxExist', arguments); }
function changeBanner() { broadcast('changeBanner', arguments); }
function resetBanner() { broadcast('resetBanner', arguments); }
...
function broadcast(funcName, args) {
    ...
    if (args[0] && typeof args[0] == 'string') {
        try {
            ...
            var str = args[0].replace(/[\u0000-\u0019]+/g, "").replace("/[\x00-\x1F\x7F-\x9F]/g", "").trim();
